...
int main(){
   ...
   processes[min_process].remaining_burst_time=1000;
   for(int i = 0; process_count!=process_done; i++){  // Gratt chart i
    for(int t = 0; t < process_count; t++){
       if((processes[min_process].remaining_burst_time>processes[t].remaining_burst_time) &&  (processes[t].remaining_burst_time>0) && (processes[t].arrival_time<=i)){
                min_process=t;
       }
    }
